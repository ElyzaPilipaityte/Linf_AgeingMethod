...
m1 <- lmer(Loo ~ method + (1|Species), data = data, REML = F)
m2 <- lmer(Loo ~ method + (1|Species) + (1|K), data = data, REML = F)
anova(m1, m2)
m3 <- lmer(Loo ~ method + K + (1|Species), data = data, REML = F)
anova(m1, m3)

m4 <- lmer(Linf_z ~ method + K_z + (1|Species), data = data, REML = F)
anova(m1, m4)
```
...
qqnorm(ranef(m4)$Species[,1])
qqline(ranef(m4)$Species[,1])
...
```{r}
ggplot(data = data) + 
  geom_point(mapping = aes(x = K_z, y = Species, color=as.factor(method)), size = 2.5)
# no outliers
...
m1 <- lmer(K_z ~ method + Linf_z + (1|Species), data = data, REML = F)
