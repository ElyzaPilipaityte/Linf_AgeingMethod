...
```{r}
Linf_plot <- ggplot(data, aes(x = Species, y = Loo, fill = method)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 55, hjust = 1, face = "italic"),
...
```{r}
library(grid)
tiff("Linf_differences.tiff", width = 300, height = 250, units = "mm", res = 300, compression = "lzw")
grid.draw(Linf_plot)
dev.off()
```
...
m4 <- lmer(Loo ~ method + (1|Species) + (1|K), data = data, REML = F)
...
Trying out models in nlme, incorporating spatial location (x, y) coordinates
...
data <- data %>% 
    mutate(method01 = if_else(method == "scales", 1, 0))
# Slightly jitter x and y to prevent exact duplicates
data$x_jittered <- jitter(data$x, factor = 1e-6)
data$y_jittered <- jitter(data$y, factor = 1e-6)

# Re-define spatial correlation structure with jittered coordinates
cor_struct <- corExp(form = ~x_jittered + y_jittered, nugget = TRUE)

# Fit the model
model <- glmmTMB(
  Loo ~ method + (1|Species),
  data = data
)
...
plotResiduals(simulationOutput, form = data$Species)
...
# 3. QQ Plot of Random Effects
ranef_vals <- ranef(model)$Species[,1]
ranef_df <- data.frame(ranef = ranef_vals)
