...
```{r}
Looic <- Compare_Growth_Models(data = perch23otoliths,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
Convergence can also be checked using a Gelman Rubin test. If all point estimates of the Gelman-Rubin diagnostic (R^
) for each parameter are less than 1.2 then convergence has been reached. Ideally these should be close to one. If not, then the number of iterations should be increased. 
```{r}
...
```{r}
Looic <- Compare_Growth_Models(data = perch23scales,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
Get_MCMC_parameters(MCMC_perchS)
```
...
ggplot(growth_both, aes(x = Age, y = LAA)) +
  # Raw points
  geom_point(data = raw_data, aes(x = Age, y = Length, color = Source), alpha = 0.3, inherit.aes = FALSE) +
  
  # Growth curve ribbons
  geom_lineribbon(aes(ymin = .lower, ymax = .upper, fill = Source, 
                      color = Source, group = interaction(Source, .width)), 
                  alpha = 0.3, linewidth = 0.8) +

  labs(y = "Total Length (cm)", x = "Age (yrs)") +

  # Custom colors for ribbons and lines
  scale_fill_manual(name = NULL, values = c("Scales" = "#E69F00", "Otoliths" = "#56B4E9")) +
  scale_color_manual(name = NULL, values = c("Scales" = "#E69F00", "Otoliths" = "#56B4E9")) +

  # Axis and theme tweaks
  scale_y_continuous(expand = c(0, 0)) +
...
Priors - got from my article averaged between two periods 1987-2023
```{r}
## Biological info - lengths in mm
max_size <- 1055
max_size_se <- 492
...
```{r}
Looic <- Compare_Growth_Models(data = sander23otoliths,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
```{r}
Get_MCMC_parameters(MCMC_sanderO)
...
Priors - got from my article averaged between two periods  cannot be zero.
The model is highly sensitive to the choice of priors; increasing the maximum size and/or se improves convergence.

Model comparisons 

```{r}
Looic <- Compare_Growth_Models(data = sander23scales,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
```{r}
Get_MCMC_parameters(MCMC_sanderS)
...
```{r}
Looic <- Compare_Growth_Models(data = roach23otoliths,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
```{r}
Get_MCMC_parameters(MCMC_roachO)
...
```{r}
Looic <- Compare_Growth_Models(data = roach23scales,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
...
```{r}
Get_MCMC_parameters(MCMC_roachS)
...
```{r}
Looic <- Compare_Growth_Models(data = bream23otoliths,
                                               stats = "LooIC",
                                               iter = 10000, 
                                               n_cores = 3,
                                               n.chains = 4,
